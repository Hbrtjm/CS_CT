...
def transpose_type(t):
    if not is_matrix(t):
        self.error(f"Transpose expects a matrix, got {t}")
    e = mat_elem(t)
    r, c = mat_shape(t)
    te = transpose_type(e) if is_matrix(e) else e
...
class TypeChecker(NodeVisitor):
    ...
    def check_unary(self, op, ty):
        ...
        if op == "!" and ty == "bool":
            return "bool"
        self.error(f"No rule for unary op '{op}' with {tstr(ty)}")
    ...
    def check_binop(self, op, lt, rt):
        ...
        if is_scalar(lt) and is_scalar(rt):
            if lt == "string" or rt == "string":
                if op == "+" and lt == "string" and rt == "string":
                    return "string"
                self.error(f"Unsupported op '{op}' for strings")
            if lt == "bool" or rt == "bool":
                self.error(f"Unsupported numeric op '{op}' for bool")
            res = self._promote_numeric(lt, rt)
            if res is None:
                self.error(f"Type mismatch: {tstr(lt)} {op} {tstr(rt)}")
            return res

        if is_matrix(lt) and is_matrix(rt):
            if op not in {"+", "-", "*", "/",
                          ".+", ".-", ".*", "./"}:
                self.error(f"Unsupported matrix op '{op}'")
            if self._matrix_broadcast(lt, rt):
                self.error(f"Matrixes can boradcast together {tstr(lt)} {op} {tstr(rt)}")
            if not self._same_shape(lt, rt):
                self.error(f"Shape mismatch for '{op}': {tstr(lt)} {op} {tstr(rt)}")
            le = base_elem(lt)
            re = base_elem(rt)
            if not (is_numeric(le) and is_numeric(re)):
                self.error(f"Matrix elements must be numeric for '{op}', got {tstr(le)} and {tstr(re)}")
            elem_res = self._promote_numeric(le, re)
            if elem_res is None:
                self.error(f"Type mismatch in elements: {tstr(le)} {op} {tstr(re)}")
            r, c = total_shape(lt)
        ...
        if is_matrix(lt) and is_scalar(rt):
            ...
            if not (is_numeric(le) and is_numeric(rt)):
                self.error(f"Unsupported op '{op}' between {tstr(lt)} and {tstr(rt)}")
        ...
        self.error(f"No rule for binary op '{op}' with {tstr(lt)} and {tstr(rt)}")



    def check_assign(self, op, ltype, rtype):
        if op == "=":
            if ltype != rtype:
                self.error(f"Cannot assign {tstr(rtype)} to {tstr(ltype)}")
            return ltype
        if op.endswith("="):
            res = self.check_binop(op[:-1], ltype, rtype)
            if res != ltype:
                self.error(f"Result of '{op}' ({tstr(res)}) not assignable to {tstr(ltype)}")
            return ltype
        self.error(f"Unknown assignment operator '{op}'")

    def expect_bool(self, ty, ctx):
        if ty != "bool":
            self.error(f"Expected bool in {ctx}, got {tstr(ty)}")
    ...
    def visit_UnaryExpr(self, node: AST.UnaryExpr):
        ...
        return self.check_unary(node.op, t)

    def visit_OpExpr(self, node: AST.OpExpr):
        lt = self.visit(node.left)
        rt = self.visit(node.right)
        return self.check_binop(node.op, lt, rt)
    ...
    def visit_Assign(self, node: AST.Assign):
        ...
        if isinstance(node.lvalue, AST.Variable):
            ...
            if ltype is None:
                ...
                ltype = rtype
            result = self.check_assign(node.operator, ltype, rtype)
    ...
    def visit_Matrix(self, node: AST.Matrix):
        ...
        for r in row_types:
            if len(r) != n_cols:
                self.error("Jagged matrix literal (rows with different lengths)")

        elem_t = row_types[0][0]
        for r in row_types:
            for t in r:
                if t != elem_t:
                    self.error(f"Inconsistent element types in matrix: {tstr(elem_t)} vs {tstr(t)}")
    ...
    def visit_MatrixIndex(self, node: AST.MatrixIndex):
        ...
        if not is_matrix(mt):
            self.error(f"[line {node.lineno}] Indexing only supported for matrices, got {tstr(mt)}")
            return None

        shape = mat_shape(mt)
        if len(node.indices) != len(shape):
            self.error(
                f"[line {node.lineno}] Wrong number of indices for matrix {node.matrix.name}: "
        ...
        for dim, (idx_expr, dim_size) in enumerate(zip(node.indices, shape)):
            ...
            if idx_t != "int":
                self.error(
                    f"[line {idx_expr.lineno}] Index {dim} for {node.matrix.name} must be int, "
                    f"got {tstr(idx_t)}"
                )
            ...
            if isinstance(idx_expr, AST.Literal) and idx_expr.typename == "int":
                ...
                if dim_size is not None and idx_val >= dim_size:
                    self.error(
                        f"[line {idx_expr.lineno}] Index {idx_val} out of bounds for dimension {dim} "
    ...
    def visit_Transpose(self, node: AST.Transpose):
        ...
        if not is_matrix(mt):
            self.error(f"Transpose expects a matrix, got {tstr(mt)}")
        return transpose_type(mt)
    ...
    def visit_Range(self, node: AST.Range):
        ...
        if st != en:
            self.error(f"Type mismatch in range bounds: {tstr(st)} vs {tstr(en)}")
        if sp is not None and sp != st:
            self.error(f"Type mismatch in range step: {tstr(st)} vs {tstr(sp)}")
        return range_t(st)

    def visit_For(self, node: AST.For):
        rng_t = self.visit(node._range)
        if not is_range(rng_t):
            self.error(f"FOR expects a range, got {tstr(rng_t)}")
    ...
    def visit_While(self, node: AST.While):
        ...
        self.expect_bool(cond_t, "while condition")
        self.st.in_loop = True
        try:
    ...
    def visit_If(self, node: AST.If):
        cond_t = self.visit(node.condition)
        self.expect_bool(cond_t, "if condition")
    ...
    def visit_Apply(self, node: AST.Apply):
        ...
        if fname in self.functions:
            ...
            for i, got in enumerate(arg_types[:len(expected)]):
                if expected[i] != got:
                    self.error(
                        f"[line {node.args[i].lineno}] Argument {i} of '{fname}' expected "
            ...
            if fname == "eye" and len(args) == 2:
                ...
                if r is not None and c is not None and r != c:
                    self.error(
                        f"[line {node.lineno}] eye expects a square shape, got {r}x{c}"
                    )
            return builder(args[0], args[1])

        else:
            if len(node.args) != 2:
                self.error(f"Operator '{fname}' expects 2 args, got {len(node.args)}")
            lt = self.visit(node.args[0])
            rt = self.visit(node.args[1])
            # lt = node.args[0]
            # rt = node.args[1]
            return self.check_binop(fname, lt, rt)
    ...
    try:
        ...
        if result and not isinstance(result, AST.Empty):
            ...
            try:
                ...
                self.error("Checker exception:", e)
            
    except IOError:
        self.error(f"Cannot open {filename} file")
        sys.exit(1)
    except Exception as e:
        self.error("Parser exception:", e)
